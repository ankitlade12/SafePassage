...
class ExitPlaybookGenerator:
    ...
    def _generate_checklist_items(
        self, location: Location, destination: Location
    ) -> List[ChecklistItem]:
    ...
    def _same_continent(self, loc1: Location, loc2: Location) -> bool:
        ...
        def get_continent(loc):
            for cont, (lon_min, lon_max, lat_min, lat_max) in continents.items():
                if (
                    lon_min <= loc.longitude <= lon_max
                    and lat_min <= loc.latitude <= lat_max
                ):
                    return cont
            return "unknown"
