...
class AnalyticsDashboard:
    ...
    def create_risk_heatmap():
        ...
        countries = [
            # High Risk
            "Ukraine", "Turkey", "Lebanon", "Syria", "Afghanistan",
            # Moderate Risk
            "Egypt", "Pakistan", "Russia", "Iran",
            # Low-Moderate Risk
            "India", "China", "Brazil", "Mexico", "South Africa",
            # Low Risk
            "United States", "United Kingdom", "France", "Germany", "Canada",
            "Australia", "Japan", "Singapore", "Switzerland", "Norway"
        ]
    ...
    def create_user_journey_timeline(user_location="Dallas"):
        ...
        events = [
            {"date": (today - timedelta(days=15)).strftime('%Y-%m-%d'), "event": "Profile Created", "type": "setup"},
            {"date": (today - timedelta(days=11)).strftime('%Y-%m-%d'), "event": "Exit Fund Activated", "type": "setup"},
            {"date": (today - timedelta(days=6)).strftime('%Y-%m-%d'), "event": "First Alert Received", "type": "alert"},
            {"date": (today - timedelta(days=1)).strftime('%Y-%m-%d'), "event": f"Location Set: {user_location}", "type": "travel"},
            {"date": today.strftime('%Y-%m-%d'), "event": "System Active", "type": "setup"}
        ]
    ...
    def show_enhanced_analytics(user_location_city="Dallas"):
        ...
        with col1:
            st.metric("Days Monitored", str(days_since_signup), delta=f"{min(days_since_signup, 7)} this week")
        with col2:
            # Realistic alert count based on days monitored
            total_alerts = days_since_signup * 2  # ~2 alerts per day
            st.metric("Total Alerts", str(total_alerts), delta=f"+{min(total_alerts, 14)} this week")
        with col3:
            st.metric("Avg Risk Level", "2.1/10", delta="-0.3")  # Dallas is low risk
        with col4:
            st.metric("Emergency Activations", "0", delta="0 today")  # Realistic - no emergencies yet
