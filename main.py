...
from models import create_sample_profile, FundStatus, PayoutMethod
from api.real_data_integration import EnhancedRiskMonitor
from core.payout_simulator import PayoutOrchestrator
from core.exit_playbook import ExitPlaybookGenerator
...
from core.crisis_scenarios import CrisisScenarioLibrary
...
if (
    st.session_state.user_profile.exit_fund
    and st.session_state.user_profile.exit_fund.status != FundStatus.ACTIVE
):
...
if "risk_monitor" not in st.session_state:
    ...
    st.session_state.risk_monitor.refresh_all_data(
...
st.caption(
    "Preparedness when minutes matter - Complete with real data integration & simulated payouts"
)
    ...
    with st.expander("✏️ Customize Profile", expanded=False):
        ...
        with col1:
            new_city = st.text_input(
                "City", value=user.current_location.city, key="profile_city"
            )
        with col2:
            new_country = st.text_input(
                "Country", value=user.current_location.country, key="profile_country"
            )
        ...
        with col3:
            fallback_city = st.text_input(
                "Fallback City",
                value=user.exit_fund.fallback_destinations[0].city
        ...
        with col4:
            fallback_country = st.text_input(
                "Fallback Country",
                value=user.exit_fund.fallback_destinations[0].country
                if user.exit_fund and user.exit_fund.fallback_destinations
                else "Greece",
        ...
        if st.button("💾 Save Profile", width="stretch"):
            ...
            fallback_lat, fallback_lon = get_coordinates(
    ...
    if user.has_active_fund():
        ...
        with st.expander("Payout Methods"):
            for method in user.exit_fund.payout_methods:
                ...
                st.caption(
                    f"ETA: {details.get('eta', 'N/A')} | Fee: {details.get('fee', 'N/A')}"
                )
    else:
...
with tab1:
    ...
    with col2:
        ...
        st.metric(
            "Fund Status",
            user.exit_fund.status.value.upper() if user.exit_fund else "N/A",
        )
        st.metric(
            "Exit Fund", f"${user.exit_fund.amount:,.0f}" if user.exit_fund else "N/A"
        )
        st.metric(
            "Fallback",
    ...
    st.plotly_chart(
        create_location_map(user.current_location, fallback, nearby_alerts),
        width="stretch",
        key="location_map",
    )
    ...
    if nearby_alerts:
        for alert in nearby_alerts:
            ...
            with st.expander(
                f"{severity_emoji} {alert.title} - {alert.get_severity_label()}",
                expanded=alert.is_critical(),
            ):
                ...
                with col1:
                    ...
                    st.write(
                        f"**Type:** {alert.risk_type.value.replace('_', ' ').title()}"
                    )
...
with tab2:
    ...
    if not user.has_active_fund():
        ...
        if transaction.status == "completed":
            completed_time = transaction.completed_at.strftime("%H:%M:%S")

            if transaction.method == PayoutMethod.CASH_PICKUP:
                st.success(f"✅ READY FOR PICKUP at {completed_time}")
                st.info(
                    f"📍 **Location:** {transaction.recipient_address}\n\n🔢 **MTCN Code:** {transaction.confirmation_code}"
                )
                st.warning("⚠️ **Bring your ID and the MTCN code to the counter.**")

            elif transaction.method == PayoutMethod.CRYPTO_WALLET:
                st.success(f"✅ CONFIRMED on Blockchain at {completed_time}")
                st.info(
                    f"🪙 **Wallet:** {transaction.recipient_address}\n\n🔗 **TxID:** {transaction.transaction_id[:10]}..."
                )
            ...
            st.warning("⏳ Payout pending... (Simulating Network Delay)")
            time.sleep(2)
            st.rerun()
        ...
        with st.expander("Transaction Details"):
            ...
            st.write(
                f"**Initiated:** {transaction.initiated_at.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            if transaction.estimated_arrival:
                st.write(
                    f"**Est. Arrival:** {transaction.estimated_arrival.strftime('%Y-%m-%d %H:%M:%S')}"
                )
        ...
        for item in critical_items:
            st.checkbox(
                item.title, key=f"critical_{item.item_id}", help=item.description
            )
        ...
        for route in checklist.safe_routes:
            st.info(
                f"**{route.method.upper()}:** {route.from_location} → {route.to_location} ({route.estimated_time})\n\n{route.notes}"
            )
        ...
        if current_risk >= 7:
            ...
            with col1:
                ...
                st.metric(
                    "Amount", f"${user.exit_fund.amount:,.0f} {user.exit_fund.currency}"
                )

                from ui.smart_payout import SmartPayoutUI
            ...
            with col_curr1:
                amount_to_convert = st.number_input(
                    "Amount",
                    value=float(user.exit_fund.amount),
                    min_value=0.0,
                    format="%.2f",
                )
            with col_curr2:
            ...
            if st.button("Convert", width="stretch"):
                ...
                st.success(
                    f"**{amount_to_convert:,.2f} {user.exit_fund.currency}** is approximately **{converted_amount:,.2f} {target_currency}**"
                )
...
with tab3:
    ...
    if st.session_state.payout_transaction:
        ...
        st.write(
            f"**{transaction.initiated_at.strftime('%Y-%m-%d %H:%M:%S')}** - Emergency protocol activated"
        )
